...
`Y`(ドラッグロック) も `H/J/K/L`(カーソル移動) も右手にあるので、
**MACFN は必ず左手で保持する**。右手上段で保持すると Y と指が衝突する。

1. 左手で上段の内側キー(T の隣) を押さえたままにする … LED2 点灯
2. 右手 H/J/K/L でカーソルを選択したい先頭へ動かす
3. 右手で **Y** を押す … Shift + 左ボタンが押された状態になる
4. 右手 H/J/K/L で動かすと、軌跡に沿ってテキストが反転していく
5. 左手を離す … 自動解除。選択範囲はそのまま残る

ドラッグロックはもう一度 Fn + Y でも解除できる。
**通常レイヤーに戻れば自動で解除される**ので、解除し忘れて
Shift が押されっぱなしになることはない。

うまくいかないときの切り分け:

- カーソルは動くが選択されない → 手順3の直後に文字を打つ。
  大文字になれば Shift は生きているのでアプリ側の問題。
- カーソルも動かない → MACFN に入れていない (LED2 を確認)。
- ターミナルなど独自の選択処理を持つアプリでは期待どおり
  動かないことがある。まず普通のテキストで確認する。
